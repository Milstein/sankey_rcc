...
const get_totals = function get_totals() {
    ...
    for (let i = 0; i < DATA.length; ++i) {
        ...
        for (let j = 1; j < FUELS.length; ++j) {
            ...
            for (let k = 0; k < BOX_NAMES.length; ++k) {
                ...
                if (j === 1 && BOX_NAMES[k] !== 'elec') {
                    ...
                    if(show_waste){
                        total[BOX_NAMES[k]] += DATA[i].waste[BOX_NAMES[k]];
                    }
                }
            }
        ...
        if(show_waste){
            total.waste = DATA[i].waste.res + DATA[i].waste.ag + DATA[i].waste.indus + DATA[i].waste.trans;
        }

        if(DATA[i].milstestone !== undefined){ 
            total.milestone = DATA[i].milstestone; 
        }else{
            total.milestone = "Not found";
        }
    ...
    return [totals, flows, labels, show_waste];
};
